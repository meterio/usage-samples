...
const verifySysContractClause = async (tx, clauseIndex) => {
  ...
  for (const evt of evts) {
    if (evt.topics[0] === transferEvtABI.signature) {
      ...
      if (!address.isAddress(receiptFrom) || !address.isAddress(receiptTo) || receiptFrom !== txOrigin) {
        continue;
      }
      return {
        verified: true,
    ...
    for (const [i, c] of tx.clauses.entries()) {
      ...
      if (!!decoded && decoded.recipient && decoded.amount) {
        ...
        if (result.verified) {
          ...
          console.log(
            `System contract transfer: ${parseInt(result.amount) / 1e18} ${result.token} from ${result.from} to ${
              result.to
            }`
          );
        } else {
